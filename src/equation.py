...
class Equation:
    ...
    def balance(self):
        if self.balanced:
            ...
            tup = tuple(left_coefficients + right_coefficients)
            left_coefficients = list(map(lambda x: int(x / reduce(gcd, tup)), left_coefficients))
            right_coefficients = list(map(lambda x: int(x / reduce(gcd, tup)), right_coefficients))
